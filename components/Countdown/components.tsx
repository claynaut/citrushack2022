        ...
        <div className='flex gap-0.5 xs:gap-1 sm:gap-2'>
          { Array.from(num).map((n, idx) =>
            <motion.div
              ...
              whileHover='hover'
              className='flex justify-center items-center w-8 xs:w-11 sm:w-14 h-8 xs:h-11 sm:h-14 rounded shadow-md cursor-default' 
            >
              {n}
            </motion.div>
          )}
        </div>
        <p className='absolute top-8 xs:top-10 sm:top-[3.25rem] text-lg xs:text-xl leading-3 font-medium'>
          {label}
        </p>
      </div>
      { separator && 
